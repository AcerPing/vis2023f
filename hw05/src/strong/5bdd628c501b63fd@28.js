...
md`# HW05_HoChePing （何哲平）_StrongBaseline_`
)}
...
function _strong1_1(d3,simple,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 2000
    ;
...
function _strong1_2(d3,data,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。 //調整組別（第一組）
  const root = d3.hierarchy(data.children[0]); 
  const links = root.links();
  const nodes = root.descendants();
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("strong1_1")).define("strong1_1", ["d3","simple","drag","url_list","invalidation"], _strong1_1);
  main.variable(observer("strong1_2")).define("strong1_2", ["d3","data","drag","url_list","invalidation"], _strong1_2);
  return main;
}
